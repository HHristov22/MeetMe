...
function validate(result) {
    ...
    let properties = [username, fullname, password, role_s, role_t, confirmPassword];
    let checkusers = 0;
    ...
    properties.forEach((property, index) => {
        ...
        if (!validityState.valid) {
            ...
            if (validityState.valueMissing) {
                ...
                let outputMissing = property.getAttribute("name");
                outputMissing = outputMissing[0].toUpperCase() + outputMissing.slice(1);
                errorSection.innerHTML = 'Please enter valid data: ' + outputMissing + '.';
                checkusers++;
            }
            ...
            if (property == role_s || property == role_t) {
                ...
                let roletoDB = ((property == role_s && x) || (property == role_t && !x)) ? "student" : "teacher";
                role_user = roletoDB;
    ...
    if (password.value != confirmPassword.value && password.value != '' && confirmPassword.value != '') {
        ...
        confirmPassword.nextElementSibling.innerHTML = "The 'Confirm password' does not match with password.";
...
function checkForSuccess(properties, toRegister, role_user){
    if(!toRegister)
    {
        properties[0].nextElementSibling.innerHTML = "This username already exists.";
